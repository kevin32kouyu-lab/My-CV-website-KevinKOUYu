...
export default function Page() {
  return (
    <main className="mx-auto max-w-6xl px-4 pb-20 sm:px-6 lg:px-8">
      <section className="grid min-h-[calc(100vh-7rem)] items-center gap-10 py-10 sm:py-14 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <Reveal>
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.2em] text-emerald-200">
              ...
              AI x Robotics x Product Delivery
            </div>
          </Reveal>
          ...
          <Reveal delay={0.06}>
            <h1 className="mt-6 text-4xl font-semibold leading-tight tracking-tight text-white sm:text-5xl lg:text-7xl">
              ...
              <span className="mt-4 block text-2xl font-medium text-zinc-200 sm:text-3xl lg:text-4xl">
          ...
          <Reveal delay={0.18}>
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <MotionDiv whileHover={{ y: -2 }} transition={{ duration: 0.18 }}>
                <Link
                  href="/projects"
                  className="group inline-flex items-center justify-center gap-2 rounded-full bg-cyan-400 px-6 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-cyan-300"
                >
                  查看项目
              ...
              <MotionDiv whileHover={{ y: -2 }} transition={{ duration: 0.18 }}>
                <a
                  ...
                  className="group inline-flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/5 px-6 py-3 text-sm font-semibold text-white transition hover:border-cyan-400/30 hover:bg-cyan-400/10"
                >
                  联系我
            ...
            {homeHighlights.map((item) => (
              <StaggerItem key={item.label}>
                <MotionDiv
                  ...
                  className="glass-panel glow-card rounded-3xl border border-white/8 p-4"
                >
        ...
        <Reveal from="scale" delay={0.18}>
          <div className="glass-panel glow-card shimmer-line rounded-[2rem] border border-white/8 p-6 sm:p-8">
            <div className="font-mono text-xs uppercase tracking-[0.26em] text-zinc-500">
              System Snapshot
            </div>
            ...
            <div className="mt-8 space-y-4">
              {homeFocusTracks.map((item, index) => (
                <MotionDiv
                  ...
                  className="glass-panel-soft rounded-[1.5rem] p-4"
                >
                  <div className="flex items-center gap-3">
                    <span className="h-2.5 w-2.5 rounded-full bg-cyan-300 shadow-[0_0_16px_rgba(103,232,249,0.75)]" />
                    <div className="font-mono text-xs uppercase tracking-[0.22em] text-cyan-200">
                      {item.label}
                    </div>
                  </div>
      ...
      <section className="space-y-6">
          ...
          {homeNarratives.map((item) => (
            <StaggerItem key={item.title}>
              ...
              >
                ...
                <div className="mt-4 text-base leading-7 text-zinc-300">{item.description}</div>
              </MotionDiv>
            </StaggerItem>
          ))}
        </StaggerGroup>
        ...
        <Reveal delay={0.12}>
          <section className="glass-panel glow-card rounded-[2rem] border border-white/8 p-6 sm:p-8">
            <div className="font-mono text-xs uppercase tracking-[0.26em] text-zinc-500">
              Current Focus
            </div>
            <div className="mt-5 flex flex-wrap gap-2">
              {coreDirections.map((item, index) => (
                <MotionSpan
                  ...
                  whileHover={{ y: -2 }}
                  className="rounded-full border border-cyan-400/18 bg-cyan-400/8 px-3 py-2 font-mono text-xs text-cyan-100"
                >
                  {item}
                </MotionSpan>
              ))}
